...
export default class UnAdminUser extends Component {
    constructor(props) {
        super(props)
        this.state = {
            user: {
    ...
    onFormSubmit = ev => {
        ...
        fetch(BASE_URL + 'user/' + APP_KEY + '/' + userId + '/roles/' + ROLE, {
            ...
            body: JSON.stringify({})
             ...
             //console.log(res)
             observer.trigger(observer.events.notification, { type: 'success', message: 'Admin has been turned into User Successfully' })
        ...
        return (
            <form className="modal-content-profile" onSubmit={this.onFormSubmit}>
                <div className="container">
                    <h1>Un Admin {this.state.user.username}</h1>
