...
postimage.upload = function(areaid) {
    ...
    window.open(postimage.opt.schema + "://" + postimage.opt.host + "/upload?" + params, "postimage", "scrollbars=1,resizable=0,width=690,height=620");
};
postimage.render = function(i) {
    ...
    icon.src = postimage.opt.schema + "://postimgs.org/sqlogo.png";
    icon.width = "16";
    icon.height = "16";
    ...
    for (var i = 0; i < areas.length; i++) {
        ...
        try {
            var dz = new Dropzone(area, {
                url: postimage.opt.schema + "://" + postimage.opt.host + "/json" + window.location.search,
                parallelUploads: 1,
                clickable: false,
                acceptedFiles: "image/*",
                maxFiles: 100,
            ...
            dz.activate = function() {
                ...
                area.style["backgroundImage"] = "url('" + postimage.opt.schema + "://postimgs.org/img/logo.png')";
            ...
            dz.on("queuecomplete", function(e) {
                ...
                xhr.open("GET", postimage.opt.schema + "://" + postimage.opt.host + "/mod?dz=1&" + params);
                xhr.onload = function() {
                    if (xhr.status === 200) {
...
postimage.init = function() {
    if (!postimage.dzimported && !/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        ...
        dzjs.src = postimage.opt.schema + "://postimgs.org/dropzone.js";
        var dzcss = document.createElement('link');
        dzcss.rel = "stylesheet";
        dzcss.href = postimage.opt.schema + "://postimgs.org/dropzone.css";
